...
bfl <- list(
  H3K27ac = here::here("data", "bam", paste0("SRR83151", 86:91, ".bam")) %>% 
    BamFileList() %>% 
    setNames(str_remove(names(.), ".bam")),
  input = here::here("data", "bam", "SRR8315192.bam") %>% 
    BamFileList() %>% 
    setNames("SRR8315192")
)
...
- There are no rules for the window sizes and spacing. Here, a window size of 180bp was chosen with a spacing of 60bp to ensure each nucleotide is covered by 3 windows
...
Now that we have alignments for >28m sliding windows across the entire genome, we might like to discard windows which are unlikely to contain H3K27ac signal.
...
This step will scan all bam files and can also be highly demanding computationally and time consuming.
This is the only step which could not be run on a 16Gb laptop, and was instead run on a high-memory server.
