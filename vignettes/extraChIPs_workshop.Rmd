...
consensus_peaks <- er_peaks %>% 
  ...
  mapByFeature(genes = subset(gtf, type == "gene"), prom = regions$promoter)
```
...
A canonical example of this is the Androgen Receptor (AR) which is sequestered in the cytoplasm in the absence of androgens, the is translocated to the nucleus upon encountering it's ligand.
Almost no binding would be expected in the absence of androgens, followed by a sigificant increase in binding.
Under these circumstances, the most appropriate normalisation would be library size only.

In order to check if there is an overall shift in signal, corresponding to global change in binding the package `r BiocStyle::Biocpkg("qantro")` [@Hicks2015-ee] can be used.
The primary test returned is for the equivalence of medians between groups, returned as the `anovaPval`


```{r qtest}
qtest <- quantro(assay(er_se, "counts"), groupFactor = er_se$treat)
qtest
```

The test results for this dataset are >> 0.05 and as such we should accept H~0~ that sample medians are equivalent between groups.
...
Taking this information we could choose many of the standard RNA-Seq normalisation methods.
The most conservative would be `method = "none"`, which corresponds to library size normalsiation, with common alternatives being TMM [@Robinson2010-qp] or RLE [@Anders2010-sd].
RLE is commonly used by the package `r BiocStyle::Biocpkg("DiffBind")` making it an established choice for ChIP-Seq data, so let's create a new assay in our `SummarizedExperiment` object called `logCPM`, using these normalisation factors.
...
Using `plotAssayRle()`, we can check RLE by individual sample, or by treatment group
...
Analysis will be performed directly on the counts using `glmQLFit()` followed by `glmQLFTest()`.
Although no demonstrated here, setting a value for the `fc` argument will switch the testing `glmQLFTreat()` which applies a range-based null-hypotesis [@McCarthy2009-qf].


```{r er-diffsig}
X <- model.matrix(~treat, data = colData(er_se))
er_diffsig <- er_se %>% 
  fitAssayDiff(design = X, norm = "RLE", asRanges = TRUE) %>% 
