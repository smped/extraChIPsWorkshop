...
The original publication containing the full detailed biological background is at https://doi.org/10.1038/s41591-020-01168-7 [@Hickey2021-mz]
...
In the interests of time, copy & pasting code will be fine for this session.
However, in the case of multiple lines connected by the pipe operator (`%>%`) participants are encouraged to break these into sections to explore the output from each step.
...
Whilst the base pipe operator (`|>`) will also be acceptable for the vast majority of code, the magrittr-pipe (`%>%`) has been used for consistency.
There are a small number of lines which take advantage of the use of `.` to represent the piped input.

If using the provided docker container, all packages will be installed.
Otherwise, please run the following, noting that it may take a few minutes to complete.

```{r}
installed <- rownames(installed.packages())
if (!"BiocManager" %in% installed) install.packages("BiocManager")
if (!"extraChIPsWorkshop" %in% installed)
  BiocManager::install("smped/extraChIPsWorkshop")
```

Please load all packages before beginning the session and alert a tutor if any errors occur.
...
This will contain not only chromosome sequences, but additional scaffold and haplotypes, which we'll choose ignore as the session progresses.

In addition, a set of annotations will have been defined describing the locations of genes, transcripts and exons, and their relationship to each other.
...
Beyond this session, many different annotation options are available through Bioconductor, either through `AnnotationHub`, or by checking the [`BiocViews` page](https://bioconductor.org/packages/release/BiocViews.html#___AnnotationData) with TxDb and EnsDb objects being particularly of interest.

## Defining the Correct Genome

The genome information can usually be obtained from the header of any associated bam files, but as an initial introduction, `extraChIPs` can also define this easily and flexibly.
...
Note that this is a trimmed down GTF especially for this workshop, and any annotations used for your own analysis may required additional editing and/or removal of features on sequences outside the provided Seqinfo object. 
...
this will classify every base in the genome defined by the `Seqinfo` object *uniquely* into one of these features.
...
Using this strategy, all genes and transcripts associated with a defined range in this object are saved in the mcols element.
...
Together, the blacklist and greylist provide a set of regions where any peaks should be excluded from analysis as a low-confidence region.
...
In addition, `importPeaks()` will silently drop any chromosomes or scaffolds not provided in the `Seqinfo` object as well.
...
Now that we've prepared the external data, we can move to the peaks we're actually interested in, which represent where our ChIP target was interacting with the genomic DNA.
...
The peaks have been prepared using `macs2` with the algorithm set to detect 'narrowPeaks' which produces files in `.narrowPeak` format. 
(The alternative is broadPeak format)
The `narrowPeak` format effectively extends the `.bed` format with the addition of key metrics defining the confidence `macs2` has in each peak.
The final column `peak` denotes the position within the provided range, where the peak signal is estimated to be.
...
*This feature makes downstream data integration far simpler* as it avoids unpredictable conflicts between peaks, genomes and annotations.
...
The convenience function, `plotGrlCol()` will plot a column from the `mcols` element of any `GRangesList`, and if using default settings, will create a boxplot the width of ranges within each element.
Given the plot is a standard `ggplot` object, we can transform the y-axis using `scale_y_log10()` using conventional syntax.

```{r plot-peak-width}
plotGrlCol(er_peaks) + scale_y_log10()
```

This figure shows a boxplot for each element of the `GRangesList`, with the total number of peaks shown as a label at the median position for every box.
...
The function enables multiple geoms (`r pander::pander(formals(plotGrlCol)$geom)`), and if we pass the samples data.frame (`df = samples$ER`), we can specify which column *exactly matches* the names of each `GRangesList` element (`.id = "id"`).
...
This represents a *time-point* which is assumed to be predictive of future changes in ER binding, i.e. the observed increase or decrease is assumed to change consistently with prolonged exposure to DHT.
This provides a useful lens through which to interpret changes in signal for this dataset.
...
Now we understand the process for a given condition, we can use `lapply` to split our `GRangesList` by treatment group, then perform the above process across both treatment groups, ensuring a peak is identified in 2/3 samples in each treatment group.
This peaks are then merged using the default (`p = 0`) to esnure all peaks are accepted and overlaid across conditions.

- Using the utility function `reduceMC()` within the function `makeConsensus()` will return each centre position as a lst column, containing all etstimated centres from individual samples. 
We'll use these mean of these individual values to estimate the overall centre position
- The function `bestOverlap()` has also been called to find which genomic region shows the most overlap with each consensus peak, thus mapping each peak to a region
- As a final step, we're using `mapByFeature()` to assign peaks to genes.
This can additionally utilise HiC (or HiChIP) interactions to confidently map long-range interactions, however at it's simplest mapping can be performed simply by providing a set of promoters and genes, with each peak being mapped to the most likely, or nearest gene, up to 100kb away from the peak.
...
Here we can take advantage of `mutate` from `r BiocStyle::Biocpkg("plyranges")` to add the chromosome names to the centre.
...
The following code chunk is how counts were prepared for this workshop noting the following steps
...
Here, the reads were single-end, so I set `pe = "none"`. The reads were extended by 175 bp as is common for ChIP-seq as this was the estimated fragment length from the macs2 output
...
We can load these pre-prepared counts in the next chunk.
...
Although not demonstrated here, setting a value for the `fc` argument will switch the testing from `glmQLFTest()` to `glmQLFTreat()` which applies a range-based null-hypothesis [@McCarthy2009-qf], or will apply the analogous strategy for the nbinomWaldTest in `DESeq2` or the `limma-trend` method.
...
In general, H3K27ac signal can range from smaller peaks of similar size to classical transcription factors, to many kb of extended signal.
...
H3K27ac signal may also change inconsistently across the entire region and within each wider 'peak', signal may change within only subsections of the region.
...
- Whilst `exclude_ranges` could be provided in the `discard` argument of the `rp` object, some read extensions do partially overlap these ranges and the subsequent removal after counting is cleaner.
- The `BamFileList` contained all 6 samples obtained using H3K27ac as the ChIP target, but also the *input sample*. One or more input samples are commonly used in ChIP-Seq and represent the background signal for each genomic range, as may be obtained with an un-targeted IgG precipitation.
...
In addition, this uses a very simple approach by taking a set of 'guide ranges' (or reference ranges) expected to contain true H3K27ac signal.
...
Setting values for `q` too high will return a highly inclusive set of windows, but may also return many ranges with only background signal.
Similarly, setting this too low return an exclusive set of windows, which may omit some windows with true signal.
Data exploration shows values on the 0.7-0.8 range to be highly suitable, however, a number of alternative strategies may also be employed, as provided by `r BiocStyle::Biocpkg("csaw")`
...
We can test this formally `quantro()` as we did for ER fixed-width peaks.
...
The `anovaPvalue` is again >> 0.05 and we should accept H~0~, that there is no difference in group medians.
This can be interpreted to mean that there is no difference overall signal between treatment groups, and as such, methods like TMM and RLE are plausible options.
However, for the purposes of demonstrating a less-common alternative, let's use Smooth Quantile Normalisation (SQN) [@Hicks2018-uw].
...
Let's normalise the logCPM values using SQN, then use these for differential signal analysis using `limma-trend` [@Law2014-xq].
...
The relationship between probes and probeset in 3' arrays is somewhat analogous to sliding windows which are eventually summarised by merging overlapping windows.
...
Any method directly provided by csaw is also viable for this step and the `extraChIPs` wrappers are simply to provide consistency in the output structures within the package.
