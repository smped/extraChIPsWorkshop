...
1. `mergeByCol()` allows selection of any column to choose a *representative window* for the region, such as the maximum/minimum 'logCPM', or the closest to the mean/median vvalues for that column
...
Overall, there seems to be a little noise at the low end, but overall the bias appears non-existent across the range of signal.
...
As we can see, *FLNB* and *PDZD2* map the same ER peak to two H3K27ac regions, suggesting these may accour in a valley between two H3K27ac peaks.
...
We won't be including anh HiC Interactions, but now we have the "FG" already provided for the HFGC track order.
